        ...
        <div className="dash-grid dash-fade-in">
            ...
            <div className="col-span-12 card recommendation-banner">
                ...
                <div style={{ position: 'relative', zIndex: 2 }}>
                    ...
                    <p style={{ color: 'rgba(255,255,255,0.8)', maxWidth: '800px', marginBottom: '20px', lineHeight: '1.6' }}>
                        Based on Prophet & LSTM time-series forecasts for your <strong>{farmArea.toFixed(1)}-acre</strong> calculated farm boundary, sustained rainfall onset is predicted optimally around <strong>{rainfallOnset?.date || 'Mar 15'}</strong>.
                        Given your <strong>{soilType}</strong> soil and <strong>{waterSource}</strong> setup, land preparation must be completed by next week to achieve an optimal germination probability.
                    </p>
            ...
            <div className="col-span-3 card">
                ...
                <div className="metric">
                    <span className="metric-value">{rainfallOnset?.date || 'Mar 15'}</span>
                    <span className="metric-label">LSTM Predicted Start</span>
                    <div className="trend-up" style={{ marginTop: '8px' }}>
                        <CheckCircle2 size={16} /> Confidence ({rainfallOnset?.confidence || 88}%)
                    </div>
                </div>
            </div>
            ...
            <div className="col-span-3 card">
                ...
                <div className="metric">
                    <span className="metric-value" style={{ color: (germinationRisk || 8) < 15 ? '#00E396' : '#FF5630' }}>{germinationRisk || 8}%</span>
                    <span className="metric-label">Probability of Failure</span>
                    <div className="trend-up" style={{ marginTop: '8px' }}>
                        {(germinationRisk || 8) < 15 ? 'Low failure risk profile' : 'Elevated risk profile'}
                    </div>
                </div>
            </div>
            ...
            <div className="col-span-3 card">
                ...
                <div className="metric">
                    <span className="metric-value">{soilMoisture || 42}%</span>
                    <span className="metric-label">Current Sub-surface saturation</span>
                    <div className="trend-up" style={{ marginTop: '8px', color: (soilMoisture || 42) > 40 ? '#00E396' : '#FFB020' }}>
                        <Activity size={16} /> {(soilMoisture || 42) > 40 ? 'Ideal sub-surface baseline' : 'Tracking towards 50%+ ideal'}
                    </div>
                </div>
            </div>
            ...
            <div className="col-span-3 card">
                ...
                <div className="metric">
                    <span className="metric-value" style={{ color: (yieldRiskAlert || 22) < 25 ? '#00E396' : '#FFB020' }}>{yieldRiskAlert || 22}%</span>
                    <span className="metric-label">Mid-season drought probability</span>
                    <div className="trend-down" style={{ marginTop: '8px', color: (yieldRiskAlert || 22) < 25 ? '#00E396' : '#FFB020' }}>
                        <AlertTriangle size={16} /> {(yieldRiskAlert || 22) < 25 ? 'Low mid-season risk' : 'Moderate alert in Flowering stage'}
                    </div>
                </div>
            </div>

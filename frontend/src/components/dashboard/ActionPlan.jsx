...
const ActionPlan = ({ farmLocation, waterSource, selectedCrop, setSelectedCrop, cropSuitability, soilType, actionPlan }) => {
    const getPhaseBorderColor = (status) => {
        if (status === 'In Progress') return '#A0AEC0';
        if (status === 'High Priority') return '#00E396';
        return '#008FFB';
    };

    const getPhaseBg = (status) => {
        if (status === 'In Progress') return 'rgba(255,255,255,0.02)';
        if (status === 'High Priority') return 'rgba(0,227,150,0.05)';
        return 'rgba(0,143,251,0.05)';
    };
        ...
        <div className="dash-grid dash-fade-in">
            <div className="col-span-12 card print-card">
                ...
                <p style={{ color: 'var(--text-muted)', marginBottom: '30px', marginTop: '15px' }}>
                    ...
                    Factors in your {waterSource} constraints to maximize the optimal germination probability window.
                </p>

                <div style={{ marginTop: '10px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                    {actionPlan && actionPlan.length > 0 ? (
                        actionPlan.map((phase, idx) => (
                            <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '24px', background: getPhaseBg(phase.status), borderRadius: '12px', borderLeft: `4px solid ${getPhaseBorderColor(phase.status)}` }}>
                                <div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                                        <span style={{ background: getPhaseBorderColor(phase.status), color: '#000', padding: '2px 8px', borderRadius: '12px', fontSize: '0.75rem', fontWeight: 'bold' }}>PHASE {phase.phase_num || (idx + 1)}</span>
                                        <h4 style={{ fontSize: '1.2rem', color: '#fff', margin: 0 }}>{phase.title}</h4>
                                    </div>
                                    <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', maxWidth: '600px' }}>{phase.description}</p>
                                </div>
                                <div style={{ textAlign: 'right', minWidth: '150px' }}>
                                    <span style={{ display: 'block', fontSize: '1.1rem', fontWeight: 600, color: getPhaseBorderColor(phase.status) }}>{phase.status}</span>

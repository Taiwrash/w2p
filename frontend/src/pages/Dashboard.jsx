...
const Dashboard = () => {
    ...
    // AI Analysis States
    const [cropSuitabilityState, setCropSuitabilityState] = useState(cropSuitability);
    const [rainfallForecastState, setRainfallForecastState] = useState(rainfallForecast);
    const [yieldProjectionsState, setYieldProjectionsState] = useState(yieldProjections);
    const [actionPlanState, setActionPlanState] = useState([]);
    const [rainfallOnset, setRainfallOnset] = useState({ date: 'Mar 15', confidence: 88 });
    const [germinationRisk, setGerminationRisk] = useState(8);
    const [soilMoisture, setSoilMoisture] = useState(42);
    const [yieldRiskAlert, setYieldRiskAlert] = useState(22);
    const [weatherSummary, setWeatherSummary] = useState(null);

    // UI State
    const [analyzing, setAnalyzing] = useState(false);

    // Fetch initial farm status from backend on load
    useEffect(() => {
        const fetchStatus = async () => {
            try {
                const res = await fetch(`${API_BASE}/api/farm-status`);
                const data = await res.json();
                if (data.lat !== undefined) {
                    setFarmLocation({ lat: data.lat, lng: data.lng });
                    setFarmArea(data.acres);
                    ...
                    if (ins) {
                        setCropSuitabilityState(ins.crop_suitability || []);
                        setRainfallForecastState(ins.rainfall_forecast || []);
                        setYieldProjectionsState(ins.yield_projections || []);
                        setActionPlanState(ins.action_plan_phases || []);
                        setRainfallOnset({
                            date: ins.rainfall_onset_date || 'Mar 15',
                            confidence: ins.rainfall_onset_confidence || 88
                        });
                        setGerminationRisk(ins.germination_risk || 8);
                        setSoilMoisture(ins.soil_moisture_est || 42);
                        setYieldRiskAlert(ins.yield_risk_alert || 22);
                        if (ins.weather_data) setWeatherSummary(ins.weather_data);
                    }
                }
            } catch (err) {
                console.error("Error loading initial farm status:", err);
            }
        };
        fetchStatus();
    }, []);
    ...
    const triggerAnalysis = async () => {
        setAnalyzing(true);
        try {
            const res = await fetch(`${API_BASE}/api/analyze-farm`, {
                method: 'POST',
            ...
            console.error("Error analyzing farm parameters:", err);
        } finally {
            setAnalyzing(false);
        }
    };
        ...
        switch (activeTab) {
            ...
            case 'overview':
                return <Overview
                    ...
                    cropSuitability={cropSuitabilityState}
                    yieldProjections={yieldProjectionsState}
                    handleNav={handleNav}
                    rainfallOnset={rainfallOnset}
                    germinationRisk={germinationRisk}
                    soilMoisture={soilMoisture}
                    yieldRiskAlert={yieldRiskAlert}
                />;
            case 'map':
                return <FarmSetup
            ...
            case 'climate':
                return <ClimateForecast
                    soilType={soilType}
                    rainfallForecast={rainfallForecastState}
            ...
            // Agent
            case 'portfolio': return <AgentPortfolio actionPlan={actionPlanState} />;
